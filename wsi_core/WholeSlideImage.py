...
class WholeSlideImage(object):
	...
	def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
							filter_params={'a_t':100}, ref_patch_size=512, exclude_ids=[], keep_ids=[], seg_downscale=1):
		"""
		...
		img = img.resize((int(w/seg_downscale[0]), int(h/seg_downscale[1])))
		...
		scale = tuple(np.array(self.level_downsamples[seg_level]) * np.array(seg_downscale))
		...
		#print([cv2.contourArea(cont) for cont in self.contours_tissue])
		#self.contours_tissue = self.contours_tissue
		#self.holes_tissue = self.holes_tissue
	...
	def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
		...
		if not view_slide_only:
			...
			if self.contours_tumor is not None and annot_display:
				...
				#cv2.drawContours(img, self.scaleContourDim(self.contours_tumor, scale), 
								 #-1, annot_color, -1, lineType=cv2.LINE_8)
	...
	def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
		...
		# process_dict = {'cont': cont, 'holes': contour_holes, 'patch_size': ref_patch_size[0], 
		# 'cont_check_fn': cont_check_fn, 'use_padding': use_padding}
		# coord_gen = coord_generator(start_x, stop_x, step_size_x, start_y, stop_y, step_size_y, args_dict=process_dict)
		# while True:
  #       	running = pool.map(savePatchIter, itertools.islice(patch_gen, n_gen_slice))
  #           if running: 
  #           	continue
  #           else: 
  #           	break
	...
	def visHeatmap(self, scores, coords, vis_level=0, 
				   ...
				   annot_color=(250, 218, 94), line_thickness=4, max_size=None,
				   custom_downsample = 1,
				   cmap='coolwarm', adjust=0.0):
		...
		if blur:
			# overlay = cv2.GaussianBlur(overlay,tuple((patch_size * (1-overlap) * 2 +1).astype(int)),0)    
		...
		# 	contours_tumor = self.scaleContourDim(self.contours_tumor, scale)
		# 	#for idx, contour in enumerate(contours_tumor):
		# 		 #contours_tumor[idx] = contour - top_left
		# 	offset = tuple(-(np.array(top_left) * scale).astype(int))
		# 	cv2.drawContours(img, contours_tumor, 
		# 					 -1, annot_color, line_thickness, offset=offset, lineType=cv2.LINE_8)
