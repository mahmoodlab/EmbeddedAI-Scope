...
def sample_rois(scores, coords, k=5, mode='range_sample', seed=1, score_start=0.45, score_end=0.55, top_left=None, bot_right=None):
    ...
    asset = {'sampled_coords': coords, 'sampled_scores': scores, 'sampled_ids': sampled_ids}
    return asset
...
def SamplePatches(coords_file_path, save_file_path, wsi_object, 
    ...
    np.random.seed(seed)
    indices = np.random.choice(np.arange(len(coords)), min(len(coords)//2, sample_num), replace=False)
...
# def FilterSampledPatches(img_file_path, stitch=True, verbose=1):
#     file = h5py.File(img_file_path, 'r+')
#     dset = file['imgs']
#     imgs = dset[:]
#     if verbose>0:
#         print('in .h5 file: total number of patches: {}'.format(len(imgs)))
#         print('in .h5 file: patch shape: {}'.format(imgs[0].shape))
        
#     if stitch:
#         canvas = Mosaic_Canvas(patch_size=imgs[0].shape[0], n=len(imgs), downscale=4, n_per_row=10, bg_color=(0,0,0), alpha=-1)
#     else:
#         canvas = None
    
#     mask = np.full((len(imgs)), True)
#     for idx in range(len(dset)):
#         patch = Image.fromarray(imgs[idx])
#         if isBlackPatch_S(patch, rgbThresh=20, percentage=0.05) or isWhitePatch_S(patch, rgbThresh=220, percentage=0.25):
#             mask[idx] = False
#             continue
...
#     imgs = imgs[mask]
#     if verbose>0:
#         print('in new .h5 file: total number of patches: {}'.format(len(imgs)))
#     dset = file.create_dataset('imgs', data=imgs)
#     file.close()
#     return canvas
