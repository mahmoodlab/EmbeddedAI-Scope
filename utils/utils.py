...
def collate_MIL_mtl(batch):
	...
	num_labels = len(batch[0]) - 1
	labels = [torch.LongTensor([item[1+i] for item in batch]) for i in range(num_labels)]
	return [img] + labels
...
collate_dict = {'MIL': collate_MIL, 'MIL_mtl': collate_MIL_mtl}
