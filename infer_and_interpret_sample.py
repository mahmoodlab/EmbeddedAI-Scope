...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
	...
	ids = ids.cpu().numpy().flatten().tolist()
	...
	for i, slide_id in enumerate(process_stack.slide_id.unique()):
		...
		for sample in samples:
			if sample['sample']:
				...
				for idx, (s_coord, s_score, s_id) in enumerate(zip(sample_results['sampled_coords'], 
															   sample_results['sampled_scores'],
															   sample_results['sampled_ids'])):
					img_idx = np.digitize(s_id, cum_counts) - 1
					img_name = image_names[img_idx]
					img_path = os.path.join(slide_path, img_name)
					wsi = openslide.open_slide(img_path)
					print('coord: {} score: {:.3f} id: {}'.format(s_coord, s_score, s_id))
					if patch_args.custom_downsample > 1:
						patch =wsi.read_region(tuple(s_coord), patch_args.patch_level, (patch_args.patch_size * patch_args.custom_downsample,) * 2).convert('RGB')
						patch = patch.resize((patch_args.patch_size,) * 2)
					else:
						patch = wsi.read_region(tuple(s_coord), patch_args.patch_level, (patch_args.patch_size,) * 2).convert('RGB')
					patch.save(os.path.join(sample_save_dir, '{}_{}_x_{}_y_{}_label_{}_pred_{}_a_{:.3f}.png'.format(idx, slide_id, s_coord[0], s_coord[1], label, Y_hats_str[0], s_score)))
