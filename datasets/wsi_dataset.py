...
class Wsi_Region(Dataset):
    ...
    def __init__(self, wsi_object, top_left=None, bot_right=None, level=0, 
        ...
        if self.custom_downsample > 1:
            ...
            step_size = tuple((np.array(step_size) * np.array(self.ref_downsample) * custom_downsample).astype(int))
        ...
        if wsi_object.contours_tissue is None:   
                    # fetch default values for x, y coordinate ranges
            if top_left is None:
                x1, y1 = (0, 0)
            else:
                x1, y1 = top_left
            ...
            x_range = np.arange(x1, x2, step=step_size[0])
            y_range = np.arange(y1, y2, step=step_size[1])
    ...
    def ret_img_toggle(self,ret_img=True):
